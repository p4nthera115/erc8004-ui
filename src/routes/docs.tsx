              ...
              return (
                <li key={item.slug}>
                  <Link
                    ...
                    activeProps={{ className: 'w-full flex items-center gap-2 px-3 py-1.5 rounded-md text-sm transition-colors text-left bg-accent/15 text-accent font-medium' }}
                  >
                    {item.label}
...
export const Route = createFileRoute('/docs')({
  beforeLoad: ({ location }) => {
    if (location.pathname === '/docs' || location.pathname === '/docs/') {
      throw redirect({ to: '/docs/components/$slug', params: { slug: 'fingerprint-badge' } })
    }
  },
  component: () => (
