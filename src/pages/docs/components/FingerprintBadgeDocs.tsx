...
const PROPS = [
  {
    ...
    required: true,
    default: "—",
    description: "Agent registry in eip155:{chainId}:{address} format",
  },
  {
    name: "agentId",
    type: "number",
    required: true,
    default: "—",
    description: "The agent's ID within the registry contract",
  },
  {
    name: "size",
    type: "number",
    required: false,
    default: "200",
    description: "Width and height of the badge in pixels",
  },
  {
    name: "className",
    type: "string",
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-col gap-3 pb-8 border-b border-border-subtle">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-medium text-green bg-green/10 px-2 py-1 rounded">
            Live
          </span>
        </div>
        <p className="text-text-secondary leading-relaxed">
          Deterministic visual identity generated from an agent's on-chain
          identifier. A FNV-1a hash of{" "}
          <code className="text-accent font-mono text-sm">
            agentRegistry + agentId
          </code>{" "}
          drives SVG dithering parameters — every agent gets a unique,
          unreplicable visual fingerprint.
        </p>
      </div>
          ...
          <div className="border-t border-border-subtle px-5 py-4 flex flex-col sm:flex-row gap-4 bg-surface">
            <div className="flex flex-col gap-1.5 flex-1">
              ...
              <div className="flex flex-wrap gap-2">
                {DEMO_AGENTS.map((a) => {
                  const isActive =
                    a.registry === activeAgent.registry &&
                    a.id === activeAgent.id
                  return (
                    <button
                      key={`${a.registry}:${a.id}`}
...
/>`}
              ...
              {PROPS.map((prop) => (
                <tr key={prop.name} className="bg-surface-raised">
                  <td className="px-4 py-3">
                    ...
                    {prop.required && (
                      <span className="ml-1.5 text-[10px] text-red font-medium">
                        required
                      </span>
                    )}
                  </td>
      ...
      <div className="flex flex-col gap-4">
        ...
        <p className="text-sm text-text-secondary">
          Exported from{" "}
          <code className="text-accent font-mono">@erc8004/ui</code>. Pure SVG —
          no canvas, no WebGL, no extra dependencies.
        </p>
      ...
      <div className="flex flex-col gap-4 pb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div
              key={item.step}
              className="rounded-xl border border-border-subtle bg-surface-raised p-5 flex flex-col gap-2"
            >
