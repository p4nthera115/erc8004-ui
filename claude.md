...
**Primary consumers are AI coding agents** (Claude Code, Cursor, etc.) — the ERC-8004 ecosystem is predominantly backend developers who use AI for frontend work. AI-agent compatibility is a first-class design goal, served via the MCP server, component registry, and `llms.txt`.
...
Every component fetches its own data. No context wrappers, no global agent state. A developer drops a component in, passes `agentRegistry` + `agentId`, and it works.
...
The app root wraps with `<QueryClientProvider>` for caching — this is infrastructure, not a data provider. It holds no agent or chain state.
...
### Shared Data Hooks
All hooks use `useQuery` from `@tanstack/react-query`. Query keys follow the pattern `[resource, agentRegistry, agentId]`.

- `useAgent(agentRegistry, agentId)` — fetches identity + registration file
- `useReputation(agentRegistry, agentId)` — fetches reputation data
...
3. Data fetching layer — `lib/fetchers/`, hooks ← implement fetchers
4. Fingerprint Badge — SVG-based, stable ✓
